...
// === Paramètres ===
const apiKey = "4b73125583da9dd12c2bfbe4bd0573f5";
const ville = "Paris";
...
fetch(url)
  ...
  .then(data => {
    ...
    document.getElementById("autres").textContent =
      `Vent : ${data.wind.speed} m/s | Humidité : ${data.main.humidity}%`;
  })
  .catch(error => {
    console.error("Erreur :", error);
    document.getElementById("description").textContent = "Impossible de charger la météo 😞";
  });
...
fetch(url)
  ...
  .then(data => {
    ...
    console.log("Météo :", description, temp + "°C");

    let condition = "meteo-soleil"; // valeur par défaut

    // --- Conditions météo ---
    if (description.includes("pluie") || description.includes("bruine") || description.includes("orage")) {
      condition = "meteo-pluie";
    } else if (temp <= 10) {
      condition = "meteo-froid";
    } else if (description.includes("nuage") || description.includes("clair") || description.includes("soleil")) {
      condition = "meteo-soleil";
    }

    console.log("Condition détectée :", condition);

    // --- Affiche les bons plats ---
    const plats = document.querySelectorAll(".grid > div");
    plats.forEach(plat => {
      // cache tous les plats d'abord
      plat.style.display = "none";

      // montre ceux qui correspondent à la météo
      if (plat.classList.contains(condition)) {
        plat.style.display = "block";
      }
    });

    // --- Mettre à jour le titre ---
    const titre = document.querySelector(".heading_container h2");
    if (titre) {
      if (condition === "meteo-soleil") titre.textContent = "☀️ Menu fraîcheur du jour";
      if (condition === "meteo-pluie") titre.textContent = "🌧️ Menu cocooning du jour";
      if (condition === "meteo-froid") titre.textContent = "❄️ Menu réconfortant du jour";
    }
  })
  .catch(error => {
